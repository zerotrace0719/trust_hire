  ...
  handleEvent: (e) => {
    ...
    switch (e.type) {
      ...
      case "agent_verdict": {
        ...
        // Alerts for flagged agents - dedupe by flag name within the same event
        const existingFlagsThisEvent = new Set(
          state.alerts
            .filter((a) => a.id.startsWith(e.event_id))
            .map((a) => a.title.toLowerCase())
        );
        const newAlerts: AlertItem[] = (e.flags || [])
          .filter((f: string) => {
            const title = f.replace(/_/g, " ");
            return !existingFlagsThisEvent.has(title.toLowerCase());
          })
          .map((f: string, i: number) => ({
            id: `${e.event_id}-${e.agent}-${f}-${i}-${uniq}`,
            ts: t,
